...
def find_concentrations(trajectory_times, concentrations):
    traj_df = pd.DataFrame({
        "trajectory_time": pd.to_datetime(trajectory_times),
        "trajectory_id": np.arange(len(trajectory_times)),
    })
    ...
    matched = matched[matched["concentration"].notna()]
    matched["trajectory_time"] = matched["trajectory_time"].dt.round("s")
    return matched[["trajectory_time", "concentration"]]
